...
def get_landmarks(img):
	...
	selected_keypoint_indices = [
		127, 93, 58, 136, 150, 149, 176, 148, 152, 377, 400, 378, 379, 365, 288, 323, 356, 70, 63, 105, 66, 55,
...
while True:
	...
	points2 = get_landmarks(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
	...
	for k in range(0, len(points2)):
		...
		points2[k] = fbc.constrain_point(points2[k], frame.shape[1], frame.shape[0])
	...
	for idx, filter in enumerate(filters):
		...
		if filter['morph']:
			...
			for i in range(0, len(dt)):
				...
				fbc.warp_triangle(img1, warped_img, t1, t2)
				fbc.warp_triangle(img1_alpha_mask, mask1, t1, t2)
			...
			tform = fbc.similarity_transform(list(points1.values()), dst_points)
	...
	cv2.putText(frame, "Press N to change filters", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 0, 0), 1)
